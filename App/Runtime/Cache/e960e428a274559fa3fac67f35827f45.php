...
if(!isNaN('<?php echo ($eq); ?>')){
  ...
  <div class="box box-solid">
    <div class="box-header with-border">
      <h3 class="box-title">添加/编辑文章</h3><a href="<?php echo U('Sys/artlist');?>" class='btn btn-default btn-xs pull-right'>返回列表</a>
    </div><!-- /.box-header -->
    <div class="box-body">
         <form method="POST" action="<?php echo U('Sys/addart');?>" id='form'>
          <input name='id' class='hide' value='<?php echo ($info["id"]); ?>'>
            ...
            <option value='<?php echo ($info["cateid"]); ?>'><?php echo (($info["catename"])?($info["catename"]):"暂无分类"); ?></option>
              <?php if(is_array($cates)): $i = 0; $__LIST__ = $cates;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ca): $mod = ($i % 2 );++$i; if($ca["parid"] == 0): ?><option value='<?php echo ($ca["id"]); ?>'><?php echo ($ca["title"]); ?></option><?php endif; ?>
                <?php if(is_array($cates)): $i = 0; $__LIST__ = $cates;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cc): $mod = ($i % 2 );++$i; if($cc['parid'] == $ca['id'] ): ?><option value='<?php echo ($cc["id"]); ?>'>┠<?php echo ($cc["title"]); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; endforeach; endif; else: echo "" ;endif; ?>
            </select>
          </div>
            <div class='form-group'>
              <label>封面图片：</label>
            ...
            <div class='form-group'>
              <label>显示模板：</label>
              <input type='text' name='tmp' class='form-control' value='<?php echo ($info["tmp"]); ?>' placeholder="文章显示的模板留空为默认">
            </div>
            <div class='form-group'>
              <label>浏览量：</label>
              <input type='number' name='views' class='form-control' value='<?php echo ($info["views"]); ?>' placeholder="">
            </div>
         </form>
    </div><!-- /.box-body -->
