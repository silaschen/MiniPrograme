...
class OCR
{
	private static $appKey = "23478002";
    private static $appSecret = "60b78b5e82502f307d005250beab2344";
	//协议(http或https)://域名:端口，注意必须有http://或https://
    //private static $host = "http://test.alicloudapi.com";
    public $host = "";

    #识别身份证信息(传入身份证正面图片文件路径)#
    public function ReadID($img){
    	$this->host = 'https://dm-51.data.aliyun.com';
    	$base64_img_string = $this->imgToBase64($img);
        $bodyContent = "{\"inputs\": [{\"image\": {\"dataType\": 50,\"dataValue\": \"".$base64_img_string."\"},\"configure\": {\"dataType\": 50,\"dataValue\": \"{\\\"side\\\":\\\"face\\\"}\"}}]}";
		$info = $this->doPostString('/rest/160601/ocr/ocr_idcard.json',$bodyContent);
		$info = json_decode($info,true);
		return json_decode($info['outputs'][0]['outputValue']['dataValue'],true);
    }

    #识别驾驶证 (传入驾驶证图片文件路径)#
    public function ReadJZ($img){
    	$this->host = 'https://dm-52.data.aliyun.com';
    	$base64_img_string = $this->imgToBase64($img);
        $bodyContent = "{\"inputs\": [{\"image\": {\"dataType\": 50,\"dataValue\": \"".$base64_img_string."\"}} ]}";
		$info = $this->doPostString('/rest/160601/ocr/ocr_driver_license.json',$bodyContent);
		$info = json_decode($info,true);
		return json_decode($info['outputs'][0]['outputValue']['dataValue'],true);
    }

    #识别行驶证 (传入行驶证图片文件路径)#
    public function ReadXZ($img){
    	$this->host = 'https://dm-53.data.aliyun.com';
    	$base64_img_string = $this->imgToBase64($img);
        $bodyContent = "{\"inputs\": [{\"image\": {\"dataType\": 50,\"dataValue\": \"".$base64_img_string."\"}} ]}";
		$info = $this->doPostString('/rest/160601/ocr/ocr_vehicle.json',$bodyContent);
		$info = json_decode($info,true);
		return json_decode($info['outputs'][0]['outputValue']['dataValue'],true);
    }
	...
	*/
	public function doPostString($path,$bodyContent = '') {
		//域名后、query前的部分
		//$path = "/poststring";
		$request = new HttpRequest($this->host, $path, HttpMethod::POST, $this::$appKey, $this::$appSecret);
        //设定Content-Type，根据服务器端接受的值来设置
